...
![API GUI 设置 FAST](../../../../../images/api_gui_settings_fast.png)

- **协议方言** - FAST 协议方言。必须作为第一个参数进行设置。选择后，下一个参数（**FAST 设置**）将包含所指定方言的 IP 地址。
- **FAST 设置** - 上一个参数中所选方言的 IP 地址和端口。你可以使用下面的选项快速加载设置。
- **设置文件** - 用于从交易所配置文件快速加载方言设置的选项。
- **登录名** - 登录名（用于 TCP 恢复服务）。
- **密码** - 密码（用于 TCP 恢复服务）。
