...
`TradingMode` 属性允许限制策略的交易活动而不完全停止它。这对于风险管理非常有用——例如，禁止开新仓位，同时只允许平已有仓位，或者完全阻止提交订单。
...
| `ReducePositionOnly` | 只允许减少当前仓位的订单。禁止开立新仓位和增加现有仓位。 |
| `LongOnly` | 仅允许做多头仓位。卖出仅允许用来平掉已持有的多头仓位（卖出数量不得超过当前持仓）。禁止开立空头仓位。 |
...
- **`LongOnly`** —— 如果当前仓位非正或卖出量超过当前仓位，则卖出订单被拒绝。
- **`Full`** -- 无限制。
