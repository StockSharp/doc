...
在处理流数据时，通常需要知道一个交易品种的最新状态——当前价格、订单簿、未平仓持仓。如果没有快照，要获取这些信息就需要加载和处理整个历史记录。快照系统通过保存每个对象的最新状态并在最短时间内提供访问来解决这个问题。
...
if (snapshot != null)
{
    Console.WriteLine($"最新价: {snapshot.Changes[Level1Fields.LastTradePrice]}");
}
```
