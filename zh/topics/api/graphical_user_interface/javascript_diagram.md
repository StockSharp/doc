...
一个策略被描述为一个**方案（scheme）**：一组*节点（nodes）*（诸如蜡烛图数据源、指标、条件或订单之类的元素）通过带类型的*端口（ports）*相互连接。该组件接收这个方案以及一个*调色板（palette）*（元素类型、其端口及颜色的目录），并将其绘制出来。
...
这三个块分别是：一个为**指标（Indicator）**（一条简单移动平均线）供给数据的**蜡烛图（Candles）**数据源；蜡烛图和指标的输出都被绘制在一个**图表（Chart）**元素上。这是 Designer 中最小的完整模式：产生数据、对其进行转换、再将其可视化。
...
然后导入 ES 模块——只读嵌入使用 `import { renderScheme } from '@stocksharp/diagram/embed'`，[交互式编辑器](javascript_diagram/editor.md)则使用 `import { StockSharpDiagram } from '@stocksharp/diagram'`。
...
同一组件也可以作为一个功能完整的**编辑器**运行——从调色板拖出元素、连接端口、编辑和删除节点、撤销/重做。参见[交互式编辑器](javascript_diagram/editor.md)和[事件与 API](javascript_diagram/events.md)。

## 另请参阅

- [交互式编辑器](javascript_diagram/editor.md)
