...
带状图序列会绘制上边界线和下边界线，并填充两者之间的通道。它天然适合展示包络线（Envelope）和布林带（Bollinger Bands），或任何生成价格通道的指标。
...
按照指标的要求计算 `upper`/`lower`——对于布林带，取收盘价的移动平均，再加上/减去其标准差的若干倍数。将带状图叠加在承载价格本身的折线或蜡烛图序列之上。
