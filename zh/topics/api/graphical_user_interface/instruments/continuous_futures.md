...
- 用于添加/删除乐器的按钮。
- **自动** 按钮允许你自动创建一个*连续*乐器。
- **确定**按钮完成*连续*仪器的创建。

**主要属性**

- [ContinuousSecurityWindow.Security](xref:StockSharp.Xaml.ContinuousSecurityWindow.Security) – 连续仪器
- [ContinuousSecurityWindow.SecurityStorage](xref:StockSharp.Xaml.ContinuousSecurityWindow.SecurityStorage) – 提供关于仪器信息的供应商。
