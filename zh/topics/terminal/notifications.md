...
通知系统用于在预先指定的事件发生时提醒用户。

可以针对投资组合、持仓、Level 1 和新闻等对象的多种交易事件设置通知。例如，当持仓加权平均价格超过某个值，或出现具有特定标题的新闻时触发通知。

通知可以采用以下形式：

- **Window** — 在屏幕角落显示一个小型消息弹窗。
- **Music** — 播放音乐。
- **SMS** — 通过短信发送消息。
- **Email** — 通过电子邮件发送消息。
- **Voice** — 使用计算机合成语音朗读消息。
- **Log** — 将消息发送到[日志](user_interface/logs.md)。
- **Disabled** — 不显示通知。
