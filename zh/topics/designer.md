...
2. [使用代码](designer/strategies/using_code.md)：适合偏好代码开发的有经验程序员。使用 **C#**、**F#** 或 **Python** 编写的策略通常比使用可视化设计器创建的策略运行得更快。与功能受模块限制的可视化方案不同，代码策略在创建时没有此类限制，可以实现任意算法。策略既可以直接在 **Designer** 中开发，也可以在 C\# 开发环境中编写（其中最常用的是 **Microsoft Visual Studio**），并使用面向专业交易算法开发的 C\# 程序库和 [API](api.md)。
