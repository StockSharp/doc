...
![查看和导出 截图 1](../../../images/hydra_view_export.png)
...
- 如果构建数据表时下载的数据量超过设定限制，屏幕上会出现以下窗口：![查看和导出 截图 2](../../../images/hydra_tick_limit.png)

  此时需要提高下载数据量限制。
- 如果数据来自时区与当前时区不同的数据源，可以调整时区。构建完成后，数据会按用户选择的时区显示。![查看和导出 截图 3](../../../images/hydra_tz.png)
