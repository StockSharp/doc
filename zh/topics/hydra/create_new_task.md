...
[Hydra](../hydra.md) 支持创建自定义扩展。要添加自己的扩展，请将相应的 dll 文件复制到 [Hydra](../hydra.md) 安装目录下的 **Plugins** 子文件夹。下面以创建现有数据的备份工具为例介绍开发过程。该工具已作为标准功能随程序提供，此处将其用作学习示例。

每个实用工具都必须实现 [IHydraTask](xref:StockSharp.Hydra.Core.IHydraTask) 接口，或者继承 [BaseHydraTask](xref:StockSharp.Hydra.Core.BaseHydraTask) 或 [ConnectorHydraTask\`1](xref:StockSharp.Hydra.Core.ConnectorHydraTask`1) 类：
...
创建实用工具时，一个关键步骤是实现 [BaseHydraTask.OnProcess](xref:StockSharp.Hydra.Core.BaseHydraTask.OnProcess) 方法，工具的主要运行逻辑由该方法定义。对于数据备份工具，全部备份逻辑均通过 [API](../api.md) 实现：
