...
`Volume Profile` 脚本用于分析所选时间段内成交量在各价格水平上的分布。交易者和量化分析人员可以借此直观查看主要交易活动集中在哪些价位。
