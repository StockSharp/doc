...
`Largest Candles` 脚本用于在指定时间段内，从所选证券的图表中找出成交量最大以及实体最长的蜡烛。该工具可帮助交易者和分析人员识别重要的市场事件及市场参与者的反应。

![hydra_analytics_big_candle](../../../../images/hydra_analytics_big_candle.png)

## 主要特点

脚本分析一组指定证券，从中查找成交量最大和实体最长的蜡烛，并将结果显示在两个图表中：

- **蜡烛实体长度图表**：显示开盘价与收盘价之差最大的蜡烛。
- **成交量图表**：显示在各自蜡烛周期内成交量最大的蜡烛。

## 工作流程

1. **选择证券和分析时段**：确定要分析的证券列表及时间范围。
2. **分析数据**：加载并分析历史蜡烛数据，找出各项指标最大的蜡烛。
3. **可视化结果**：在分析面板的图表中显示找到的蜡烛。

## 应用

- **市场活跃度分析**：帮助确定交易者最活跃的时点以及潜在的市场反转。
- **识别关键价位**：成交量较大且实体较长的蜡烛通常出现在关键支撑位和阻力位附近。
- **策略规划**：结合潜在波动性，利用最大蜡烛的信息规划入场点和出场点。
