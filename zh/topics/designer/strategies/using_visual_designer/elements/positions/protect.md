...
- **Price** – 当前价格（可取自K线、最近一笔逐笔成交等）。该值用于跟踪交易品种的当前价格并激活保护订单。
...
> [!WARNING]
> 传入成交不能是整个策略的成交（即[策略成交](../common/trades_by_strategy.md)模块的输出），否则会导致当前持仓计算错误：保护订单产生的成交也会成为策略成交。**Position Protection** 模块应接收 [注册订单](../orders/register.md) 和 [修改持仓](modify.md) 模块的 **Transaction** 输出端口所产生的成交，或接收来自其他直接改变持仓的类似组件的成交。
