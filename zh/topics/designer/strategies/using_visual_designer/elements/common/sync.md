...
- **输入**：连接新的数据源时，模块会自动创建对应的输出端口以及一个新的输入端口。输入值的数量不受限制。

## 参数

- **间隔**：设置数据必须更新或删除的时间间隔。如果新输入值的时间晚于前一个值的时间加上该间隔，则会清除旧数据，并开始汇集新一批数据。
- **清除项**：启用后，当所有已连接输入端口的数据收集完成时会将其清除；否则，数据会持续累积，直至出现下一个时间间隔的数据。
