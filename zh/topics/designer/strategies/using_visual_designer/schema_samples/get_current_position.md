...
在 [变量](../elements/data_sources/variable.md) 模块中选择 **Instrument** 数据类型。如果未指定交易品种，但已设置 **Common** 组中的 **Parameters** 标志，则会从策略中获取交易品种，再将其传递给 [持仓](../elements/positions/current.md)。

[持仓](../elements/positions/current.md) 模块同样没有指定持仓属性，但设置了 **Common** 组中的 **Parameters** 标志。这意味着会针对策略设置中指定的投资组合获取持仓。
