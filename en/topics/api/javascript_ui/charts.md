...
- [Candlestick](charts/candlestick.md) — classic OHLC candles.
- [OHLC bars](charts/bar.md) — open/close ticks on a vertical range bar.
- [Line](charts/line.md) — a single polyline through the closes.
- [Area](charts/area.md) — a line with a gradient fill.
- [Histogram](charts/histogram.md) — vertical bars, typically volume.
- [Band](charts/band.md) — an upper/lower channel (envelopes, Bollinger).
- [Heikin-Ashi](charts/heikin_ashi.md) — smoothed candles that filter noise.
- [Renko](charts/renko.md) — price-driven bricks, time-agnostic.
- [Point and Figure](charts/point_figure.md) — X/O columns of price movement.
- [Footprint](charts/footprint.md) — bid × ask volume at every price inside each bar.
- [Volume profile](charts/volume_profile.md) — volume-at-price with POC and value area.
- [TPO (Market profile)](charts/tpo.md) — time spent at each price per session.

Beyond the series types, the chart also has an [indicator engine](charts/indicators.md) with about 160 studies and [lazy history backfill](charts/backfill.md) that loads older bars as you scroll.
...
- [Windows chart components](../graphical_user_interface/charts.md)
