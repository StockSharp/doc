...
2. For convenient work, open and pin the **Palette** and **Properties** panels of the **Schemas** area by clicking the ![Designer Algorithm creation of cubes 13](../../../../images/designer_algorithm_creation_of_elements_13.png) button. The result is a window of the following type.
...
6. For the indicators to work, pass a candle series to them. To do this, create the [Candles](elements/data_sources/candles.md) cube. In the example, only formed candles with a timeframe of 5 minutes are used.
...
8. Add the [Chart](elements/common/chart.md) to display candles, indicators, and trades visually. Add display elements for candles, two indicators, and trades to the [Chart](elements/common/chart.md).
...
To process the current position, you can use the ready-made schema described in [Get current position](schema_samples/get_current_position.md). This schema determines the actual value of the required order volume. If the position must be reversed, it returns twice the portfolio value.
