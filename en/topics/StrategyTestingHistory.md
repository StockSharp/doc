...
The data for backtesting must be pre\-downloaded and stored in a special [S\#](StockSharpAbout.md) format. This can be done on one's own using [Connectors](API_Connectors.md) and [Storage API](StoragesApi.md), or to set up and run the special [Hydra](Hydra.md) app. 
