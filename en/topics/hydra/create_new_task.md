...
An important step in creating the utility is the implementation of the [BaseHydraTask.OnProcess](xref:StockSharp.Hydra.Core.BaseHydraTask.OnProcess), method, which implements the utility logic. In the case of implementing data backup, all the logic is inside [API](../api.md): 
