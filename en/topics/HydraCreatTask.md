...
Each utility has to implement the [IHydraTask](../api/StockSharp.Hydra.Core.IHydraTask.html) interface (or inherit from the [BaseHydraTask](../api/StockSharp.Hydra.Core.BaseHydraTask.html) or [ConnectorHydraTask\`1](../api/StockSharp.Hydra.Core.ConnectorHydraTask`1.html) classes): 
...
An important step in creating the utility is the implementation of the [BaseHydraTask.OnProcess](../api/StockSharp.Hydra.Core.BaseHydraTask.OnProcess.html), method, which implements the utility logic. In the case of implementing data backup, all the logic is inside [S\#.API](StockSharpAbout.md): 
