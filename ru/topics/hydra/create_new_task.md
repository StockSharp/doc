...
Важным этапом создания утилиты является реализация метода [BaseHydraTask.OnProcess](xref:StockSharp.Hydra.Core.BaseHydraTask.OnProcess), в котором реализуется логика утилиты. В случае реализации резервирования, вся логика находится внутри [API](../api.md): 
