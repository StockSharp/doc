...
1. Сначала необходимо создать свой тип сообщения свечи. Тип должен наследоваться от класса [CandleMessage](xref:StockSharp.Messages.CandleMessage):
   ...
   public class DeltaCandleMessage : CandleMessage
   {
      ...
      // чтобы использовать одно и то же значение и в RegisterCandleMessageType
...
2. **Регистрация типа свечей** — используйте `Extensions.RegisterCandleMessageType`, передавая туда только тип вашего сообщения свечи. Регистрация необходима для правильной интеграции с графическими контролами StockSharp и хранилищем данных. Без нее тип свечей будет работать только в коде, но не будет доступен в пользовательском интерфейсе.
...
4. **Единый идентификатор** — убедитесь, что везде, где требуется `MessageType`, используется одно и то же значение (например, константа из хелпера), включая регистрацию через `Extensions.RegisterCandleMessageType` и конструктор сообщения.
