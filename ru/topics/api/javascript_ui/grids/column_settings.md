...
Адаптер работает с ключами **в нижнем регистре**: именно так он читает `data-col` при разборе таблицы и такие ключи отдаёт наружу — из `defaultKeys()` и в `write()`.

> [!CAUTION]
> Ключи, приходящие в `apply()` и из `read()`, сверяются с этим списком **как есть, без приведения регистра**. Ключ `Time` не совпадёт с обнаруженным `time` и будет отброшен как неизвестный, а если так записаны все ключи, видимых столбцов не останется вовсе — таблица откроется без единого управляемого столбца, без ошибки в консоли. Записывайте в хранилище то, что пришло в `write()`, и не пересобирайте ключи самостоятельно.

Неизвестные и повторяющиеся ключи при применении отбрасываются. Пустой массив — это законная раскладка «не показывать ни одного управляемого столбца», а не «раскладка не выбрана»; последнее обозначается только `null`.
