...
- [DataGrid](grids/data_grid.md) строит заголовок и строки по единому описанию столбцов. Он отвечает за сортировку, фильтры, группировку, выбор строк, закреплённые итоги, контекстное меню, сохранение состояния и экспорт.
- [ColumnSettings](grids/column_settings.md) подключается к уже отрисованной сервером HTML-таблице и позволяет пользователю менять порядок и видимость столбцов.
- [TableSort](grids/table_sort.md) предоставляет самостоятельный контроллер сортировки для таблицы, которой управляет приложение.
- [TableExport](grids/table_export.md) формирует книгу OOXML `.xlsx` без сторонней библиотеки.
