...
**Коэффициент эффективности Кауфмана (коэффициент эффективности Кауфмана, KER)** - это технический индикатор, разработанный Перри Кауфманом, который измеряет эффективность ценового движения, сравнивая направленное движение цены с общей волатильностью.
...
KER был разработан Перри Кауфманом и первоначально использовался как компонент адаптивной скользящей средней Кауфмана (KAMA). Однако сам по себе KER является ценным инструментом, который помогает определить, находится ли рынок в трендовом или колебательном состоянии.
...
1. Расчет направленного движения (чистого изменения) за период:
   ```
   направление = |Price[current] - Price[current - Length]|
   ```

2. Расчет общего движения (суммы всех изменений) за период:
   ```
   волатильность = Sum(|Price[i] - Price[i-1]|) для i от (current - Length + 1) до current
   ```

3. Расчет коэффициента эффективности:
   ```
   KER = направление / волатильность
   ```
...
Если волатильность равна нулю (что маловероятно), KER принимается равным нулю для избежания деления на ноль.
...
![График индикатора KER](../../../../images/indicator_kaufman_efficiency_ratio.png)
