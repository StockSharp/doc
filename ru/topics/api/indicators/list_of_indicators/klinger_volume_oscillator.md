...
**Объемный осциллятор Клингера (KVO)** - это технический индикатор, разработанный Стефеном Клингером, который использует объем и цену для определения долгосрочных тенденций и краткосрочных разворотов на рынке.
...
KVO объединяет информацию о цене с объемом, используя показатель денежного потока объема (сила объема), который учитывает как направление, так и величину ценового движения, а также объем торгов. Затем он применяет экспоненциальные скользящие средние (EMA) с двумя разными периодами к этому денежному потоку и вычисляет разницу между ними.
...
2. Расчет показателя денежного потока объема (сила объема):
   ```
   сила объема = Volume * Trend * abs(2 * ((Close - Low) - (High - Close)) / (High - Low))
   ```
   Если (High - Low) равно нулю, то сила объема принимается равным объему, умноженному на тренд.
...
![График индикатора KVO](../../../../images/indicator_klinger_volume_oscillator.png)
