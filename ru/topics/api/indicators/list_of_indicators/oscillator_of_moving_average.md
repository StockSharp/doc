...
**Осциллятор скользящей средней (OMA)** - это технический индикатор, который измеряет разницу между двумя скользящими средними с разными периодами для определения импульса и потенциальных точек разворота.
...
1. Расчет короткой скользящей средней:
   ```
   короткая скользящая средняя = SMA(Price, ShortPeriod)
   ```

2. Расчет длинной скользящей средней:
   ```
   длинная скользящая средняя = SMA(Price, LongPeriod)
   ```

3. Расчет OMA как разницы между короткой и длинной скользящими средними:
   ```
   OMA = короткая скользящая средняя - длинная скользящая средняя
   ```
...
![График индикатора OMA](../../../../images/indicator_oscillator_of_moving_average.png)
