...
**Гармонический осциллятор (гармонический осциллятор, HO)** - это технический индикатор, основанный на теории гармонических колебаний, который помогает выявлять циклические компоненты в движении цены.
...
1. Предварительная обработка ценового ряда (детрендинг):
   ```
   цена без тренда = Price - SMA(Price, Length)
   ```

2. Применение спектрального анализа для выявления доминирующих циклов:
   ```
   спектральные компоненты = FFT(цена без тренда)
   ```

3. Выделение наиболее значимых гармонических компонентов:
   ```
   Dominant Cycles = N наиболее значимых спектральных компонентов по амплитуде
   ```

4. Синтез Гармонического осциллятора на основе доминирующих циклов:
   ```
   HO = реконструкция доминирующих циклов через обратное FFT
   ```
...
![График индикатора HO](../../../../images/indicator_harmonic_oscillator.png)
