...
**Индекс силы (FI)** - это технический индикатор, разработанный доктором Александром Элдером, который измеряет силу каждого движения цены на основе ее направления, величины и объема торгов.

Для использования индикатора необходимо использовать класс [ForceIndex](xref:StockSharp.Algo.Indicators.ForceIndex).

## Описание

Индекс силы представляет собой осциллятор, который измеряет силу "быков" (покупателей) или "медведей" (продавцов) на каждом движении цены. Он объединяет три важных элемента рыночной информации: направление движения цены, величину движения и объем торгов.
...
1. Вычисление однодневного (или одного периода) Индекса силы:
   ```
   однопериодный индекс силы = (Close[текущий] - Close[предыдущий]) * Volume[текущий]
   ```

2. Сглаживание с использованием экспоненциального скользящего среднего (EMA):
   ```
   Индекс силы = EMA(однопериодный индекс силы, Length)
   ```
...
![График индикатора FI](../../../../images/indicator_force_index.png)
