...
**Индикатор каналов Келтнера (каналы Кельтнера, KC)** - это технический индикатор, представляющий собой набор полос волатильности, который использует экспоненциальное скользящее среднее (EMA) в качестве центральной линии и средний истинный диапазон (ATR) для определения ширины канала.
...
Индикатор был разработан Честером Келтнером в 1960-х годах и позже был модифицирован Линдой Рашке, которая заменила простое скользящее среднее (SMA) на экспоненциальное скользящее среднее (EMA) и стала использовать ATR вместо диапазона максимум-минимум для расчета ширины канала.
...
1. Расчет экспоненциального скользящего среднего:
   ```
   Средняя линия = EMA(Цена, Длина)
   ```

2. Расчет среднего истинного диапазона:
   ```
   ATR = средний истинный диапазон за период Length
   ```

3. Расчет верхней и нижней полос:
   ```
   Верхняя полоса = Средняя линия + (Множитель * ATR)
   Нижняя полоса = Средняя линия - (Множитель * ATR)
   ```
...
![График индикатора KC](../../../../images/indicator_keltner_channels.png)
