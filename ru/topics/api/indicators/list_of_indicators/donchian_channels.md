...
**Каналы Дончиана (каналы Дончиана, DC)** - это технический индикатор, разработанный трейдером Ричардом Дончианом, который состоит из верхней и нижней полосы (границы канала), основанных на максимальных и минимальных значениях цены за определенный период.
...
1. Верхняя линия канала:
   ```
   верхняя линия = наивысший максимум за период Length
   ```

2. Нижняя линия канала:
   ```
   нижняя линия = наименьший минимум за период Length
   ```
...
![График индикатора DC](../../../../images/indicator_donchian_channels.png)
