...
**Индекс максимумов и минимумов (HLI)** - это технический индикатор, который измеряет отношение количества акций, достигших новых максимумов, к количеству акций, достигших новых минимумов, за определенный период времени.

Для использования индикатора необходимо использовать класс [HighLowIndex](xref:StockSharp.Algo.Indicators.HighLowIndex).

## Описание

Индекс максимумов и минимумов (HLI) является индикатором широты рынка, который анализирует общую рыночную активность, сравнивая количество инструментов, которые достигают новых максимумов, с количеством инструментов, которые достигают новых минимумов. Это позволяет оценить внутреннюю силу или слабость рынка.
...
Расчет индекса максимумов и минимумов включает следующие этапы:

1. Подсчет количества ценных бумаг, достигших новых максимумов за период Length:
   ```
   новые максимумы = Количество инструментов, достигших новых максимумов за период Length
   ```

2. Подсчет количества ценных бумаг, достигших новых минимумов за период Length:
   ```
   новые минимумы = Количество инструментов, достигших новых минимумов за период Length
   ```

3. Расчет индекс максимумов-минимумов как отношения разницы между новыми максимумами и минимумами к их сумме:
   ```
   HLI = ((новые максимумы - новые минимумы) / (новые максимумы + новые минимумы)) * 100
   ```

Примечание: Если (новые максимумы + новые минимумы) равно нулю, то HLI принимается равным нулю для избежания деления на ноль.

## Интерпретация

Индекс максимумов и минимумов интерпретируется следующим образом:
...
![График индикатора HLI](../../../../images/indicator_high_low_index.png)
