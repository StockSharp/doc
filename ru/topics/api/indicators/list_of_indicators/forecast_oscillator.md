...
**Прогнозный осциллятор (FOSC)** - это технический индикатор, который измеряет отклонение цены от ее прогнозируемого значения, полученного с помощью линейной регрессии, и представляет это отклонение в виде процентного соотношения.
...
1. Расчет прогнозной линии с использованием линейной регрессии за указанный период:
   ```
   прогноз = линия линейной регрессии(Close, Length)
   ```

2. Расчет осциллятора как процентного отношения между текущей ценой и прогнозным значением:
   ```
   FOSC = ((Close - прогноз) / прогноз) * 100
   ```

где:
- Close - текущая цена закрытия
- прогноз - прогнозное значение, полученное с помощью линейной регрессии
...
![График индикатора FOSC](../../../../images/indicator_forecast_oscillator.png)
