...
**Индикатор внутридневного момента (индекс внутридневного импульса, IMI)** - это технический индикатор, разработанный Тушаром Чанде, который объединяет принципы внутридневного ценообразования и концепцию RSI для измерения внутридневного импульса.
...
1. Определение внутридневного движения цены:
   ```
   прирост = Close - Open, если Close > Open
   снижение = Open - Close, если Close < Open
   ```

2. Расчет суммы положительных и отрицательных движений за период Length:
   ```
   сумма приростов = Сумма всех прирост за период Length
   сумма снижений = Сумма всех снижение за период Length
   ```

3. Расчет IMI по формуле, аналогичной RSI:
   ```
   IMI = 100 * (сумма приростов / (сумма приростов + сумма снижений))
   ```

Примечание: Если (сумма приростов + сумма снижений) равно нулю, то IMI принимается равным 50 для избежания деления на ноль.
...
![График индикатора IMI](../../../../images/indicator_intraday_momentum_index.png)
