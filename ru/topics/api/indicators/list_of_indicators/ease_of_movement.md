...
**Легкость движения (EMV)** - это технический индикатор, разработанный Ричардом Армсом, который соотносит изменение цены с объемом, чтобы оценить легкость, с которой цена движется вверх или вниз.
...
1. Расчет среднеточечного движения (Движение средней точки):
   ```
   Средняя точка = (High + Low) / 2
   Движение средней точки = Средняя точка[текущий] - Средняя точка[предыдущий]
   ```

2. Расчет коэффициента объема-расстояния (коэффициент объем-расстояние):
   ```
   коэффициент объем-расстояние = Volume / (High - Low)
   ```

3. Расчет одноточечного EMV:
   ```
   однопериодный EMV = Движение средней точки / коэффициент объем-расстояние
   ```

4. Сглаживание для получения окончательного EMV:
   ```
   EMV = SMA(однопериодный EMV, Length)
   ```
...
![График индикатора EMV](../../../../images/indicator_ease_of_movement.png)
