...
**Индекс спроса (DI)** - это технический индикатор, разработанный Джеймсом Сиббетом, который анализирует соотношение между ценой и объемом для оценки силы спроса и давления покупателей на рынке.
...
1. Расчет ценового компонента на основе изменения цены:
   ```
   ценовая компонента = ((High + Low + Close) / 3) - ((предыдущий максимум + предыдущий минимум + предыдущее закрытие) / 3)
   ```
...
3. Вычисление спроса как соотношения ценового и объемного компонентов:
   ```
   сырой спрос = ценовая компонента / компонента объема
   ```

4. Сглаживание полученных значений для уменьшения шума:
   ```
   сглаженный спрос = EMA(сырой спрос, Length)
   ```

5. Нормализация результата для получения окончательного индекса:
   ```
   Индекс спроса = 100 * Normalized(сглаженный спрос)
   ```
...
![График индикатора DI](../../../../images/indicator_demand_index.png)
