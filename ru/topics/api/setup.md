...
2. Выберите шаблон **Консольное приложение** или **Приложение WPF** (GUI-приложение)
3. Укажите целевой фреймворк **.NET 10**

### JetBrains Rider

1. **Файл → Новое решение**
2. Выберите **.NET / .NET Core → Консольное приложение**
3. Укажите **Целевой фреймворк: net10.0**
...
> [!NOTE]
> Полный список коннекторов — в разделе [Коннекторы](connectors.md). Некоторые коннекторы доступны только через [приватный NuGet-сервер](#private-nuget-server).
...
## Приватный NuGet-сервер {#private-nuget-server}
...
Некоторые коннекторы доступны только через [приватный сервер](#private-nuget-server). Убедитесь, что добавлен правильный источник.
...
Console.WriteLine("StockSharp успешно настроен!");

var connector = new Connector();
Console.WriteLine($"Коннектор создан: {connector}");
```
...
Готовые примеры использования StockSharp доступны в [каталоге примеров](https://github.com/stocksharp/stocksharp/tree/master/Samples) репозитория. Они охватывают подключение к биржам, подписку на данные, построение свечей, индикаторы, стратегии и тестирование.
