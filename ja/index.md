...
[StockSharp (S#)](https://stocksharp.com/store/) は、株式、先物、オプション、暗号資産、外国為替を含むグローバル市場向けの**無料**トレーディングアプリケーションを提供します。従来型のトレーディングロボットから HFT (High-Frequency Trading) システムまで、手動で取引することも、自動売買戦略を実行することもできます。

**90 以上のブローカー、取引所、データソースをサポート:** [Connectors](topics/api/connectors.md)。

S# は、利用可能なコネクタでサポートされている任意のブローカー、取引所、データソースで動作します。

> [!NOTE]
> すべてのプログラムは [Installer](topics/installer.md) ユーティリティを通じてインストールされます。
...
- [Connectors](topics/api/connectors.md) を通じて多数のデータソースをサポート。
- 高い圧縮率 (約定 1 件あたり 2 バイト、板情報 1 件あたり 7 バイト)。
- あらゆるデータ型 (キャンドル、ティック、板情報、注文ログ、オプション、ニュースなど) を処理。
- 保存済みデータへの API アクセス。
...
- クリックでチャートから直接取引できます。
- 任意の時間枠をサポートします。
- Volume、Tick、Range、Renko など、さまざまなキャンドルタイプを備えています。
