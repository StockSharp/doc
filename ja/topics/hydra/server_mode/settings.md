...
[Hydra](../../hydra.md) サーバーモードを有効にするには、プログラムの上部メニューで **Server mode** タブを選択します。

![hydra server menu](../../../images/hydra_server_menu.png)

その後、**Settings** ボタンをクリックして、サーバーモード設定ウィンドウを開きます。

![hydra server](../../../images/hydra_server.png)

**Hydra Server**

- **FIX server** - [Hydra](../../hydra.md) をサーバーモードに切り替え、FIX プロトコル経由でライブ取引データと履歴データを配信します。 
...
- **Authorization** - Hydra サーバーへのアクセス権を得るための認可 
- **Number of securities** - サーバーからリクエストできる銘柄の最大数 
- **Candles (days)** - ローソク足履歴をダウンロードできる最大日数 
- **Ticks (days)** - ティックデータ履歴をダウンロードできる最大日数 
- **Order books (days)** - 板情報履歴をダウンロードできる最大日数 
- **OL (days)** - OL データ履歴をダウンロードできる最大日数 
- **Transactions (days)** - トランザクション履歴をダウンロードできる最大日数 
- **Simulator** - シミュレーターモードをオンにします 
- **Security mapping** - 指定した銘柄のみの転送モードを有効にします。 

**Authorization** を **Anonymous** 以外に設定すると、**Common** タブに **Users** ボタンが表示されます。それをクリックすると、**Users** ウィンドウが表示されます。
