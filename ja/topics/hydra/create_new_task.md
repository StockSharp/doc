...
[Hydra](../hydra.md) は、カスタム拡張を作成できるように構築されています。独自の拡張を追加するには、[Hydra](../hydra.md) がインストールされている場所の **Plugins** サブフォルダーに dll をコピーする必要があります。以下では、既存データをバックアップするためのユーティリティを作成する手順を示します（これは標準で含まれており、教材として提示されています）。

各ユーティリティは [IHydraTask](xref:StockSharp.Hydra.Core.IHydraTask) インターフェイスを実装する必要があります（または [BaseHydraTask](xref:StockSharp.Hydra.Core.BaseHydraTask) クラス、もしくは [ConnectorHydraTask\`1](xref:StockSharp.Hydra.Core.ConnectorHydraTask`1) クラスを継承します）。
...
ユーティリティ作成における重要な手順は、ユーティリティのロジックを実装する [BaseHydraTask.OnProcess](xref:StockSharp.Hydra.Core.BaseHydraTask.OnProcess) メソッドの実装です。データバックアップを実装する場合、すべてのロジックは [API](../api.md) 内にあります。
