...
- -h - 履歴データを含むディレクトリへのパス。[server](../hydra_server.md) モードを使用する場合はネットワークアドレスにすることもできます。
- --hl - （任意）[server](../hydra_server.md) モードで使用されるログイン。
- --hp - （任意）[server](../hydra_server.md) モードで使用されるパスワード。
