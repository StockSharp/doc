...
- **Compositions** は、[Schemas](../user_interface/schemas.md) パネルの **Composite elements** フォルダーにあるすべてのブロックを XML ファイルとして保存します。このディレクトリからファイルを削除すると、対応する **Composite element** が [Schemas](../user_interface/schemas.md) パネルの **Composite elements** フォルダーから削除されます。これらのファイルを手動で編集しないでください。編集すると、対応する **Composite elements** ブロックが破損する可能性があります。
- **LiveStrategies** は、[Schemas](../user_interface/schemas.md) パネルの **Trading** フォルダーにあるすべてのブロックを XML ファイルとして保存します。このディレクトリからファイルを削除すると、対応するストラテジーが [Schemas](../user_interface/schemas.md) パネルの **Trading** フォルダーから削除されます。これらのファイルを手動で編集しないでください。編集すると、対応するストラテジーが破損する可能性があります。
- **Logs** には、すべての [Designer](../../designer.md) クラッシュログが含まれ、[Designer](../../designer.md) のトラブルシューティングを簡単にします。
- **SourceCode** は、[Schemas](../user_interface/schemas.md) パネルの **Source code** フォルダーにあるすべてのブロックを XML ファイルとして保存します。このディレクトリからファイルを削除すると、**Source code** ブロックが [Schemas](../user_interface/schemas.md) パネルの **Source code** フォルダーから削除されます。これらのファイルを手動で編集しないでください。編集すると、対応する **Source code** ブロックが破損する可能性があります。
- **Strategies** は、[Schemas](../user_interface/schemas.md) パネルの **Strategies** フォルダーにあるすべてのブロックを XML ファイルとして保存します。このディレクトリからファイルを削除すると、対応するストラテジーが Schemas パネルの **Strategies** フォルダーから削除されます。これらのファイルを手動で編集しないでください。編集すると、対応するストラテジーが破損する可能性があります。このフォルダーにストラテジーファイルを手動で追加して [Designer](../../designer.md) を再起動すると、そのストラテジーは [Schemas](../user_interface/schemas.md) パネルの **Strategies** フォルダーに表示されます。
- **Storage** には、[Designer](../../designer.md) が対応する [マーケットデータストレージ](../market_data_storage.md) にダウンロードしたマーケットデータが含まれます。このフォルダーは [マーケットデータストレージ](../market_data_storage.md) の作成時に作成され、既定のパスはこのフォルダーを指します。このフォルダーを削除すると、対応するストレージからすべてのダウンロード済みマーケットデータが削除されます。ストレージに CSV ファイルが含まれている場合、それらは標準のテキストエディターまたは MS Excel で編集できます。BIN ファイルを手動で編集することはできません。
