...
> [!TIP]
> Windows では、Hydra Server を Windows サービスとして登録し、システム起動時に自動的に起動できます。詳細については、[Windows サービス](https://en.wikipedia.org/wiki/Windows_service) を参照してください。
...
- **CompanyPath** - プログラムを Windows サービスとして使用する場合は、"C:\\Users\\%user_name%\\Documents\\StockSharp" のようなパスを設定する必要があります。
