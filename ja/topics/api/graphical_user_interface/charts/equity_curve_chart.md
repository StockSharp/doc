...
[EquityCurveChart](xref:StockSharp.Xaml.Charting.EquityCurveChart) は、エクイティカーブを表示するためのグラフィカルコンポーネントです。
...
1. [EquityCurveChart](xref:StockSharp.Xaml.Charting.EquityCurveChart) グラフィカルコンポーネントを XAML に追加します。コンポーネントに **Curve** という名前を割り当てます。
   ...
   ```
2. メインウィンドウのコードで、[EquityCurveChart.CreateCurve](xref:StockSharp.Xaml.Charting.EquityCurveChart.CreateCurve(System.String,System.Windows.Media.Color,System.Windows.Media.Color,Ecng.Drawing.DrawStyles,System.Guid))**(**[System.String](xref:System.String) title, [System.Windows.Media.Color](xref:System.Windows.Media.Color) color, [System.Windows.Media.Color](xref:System.Windows.Media.Color) secondColor, [Ecng.Drawing.DrawStyles](xref:Ecng.Drawing.DrawStyles) style, [System.Guid](xref:System.Guid) id **)** メソッドを使用して、チャートを描画するためのデータソースを作成します。
   ...
   ```
3. ストラテジーの PnL 値が変更されたとき、データソースにデータを追加します。この場合は、特殊な [ChartDrawData](xref:StockSharp.Xaml.Charting.ChartDrawData) クラスを使用します。
