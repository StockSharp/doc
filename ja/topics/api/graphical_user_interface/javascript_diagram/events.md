...
ダイアグラムコンポーネントは、意図的に UI の装飾（クローム）を持ちません。データを運ぶイベントを発行し、メニューやダイアログの描画はホスト側に委ね、モデルを操作するためのメソッドを公開します。プロパティパネルやコンテキストメニューが、コンポーネントのイベントに配線した *あなた自身の* UI になるのはこのためです。
