...
`BollingerStrategyUpBandStrategy` は、[BollingerBands](xref:StockSharp.Algo.Indicators.BollingerBands) インジケーターに基づくストラテジーです。価格が Bollinger Bands の上限に到達したときにロング ポジションを開き、価格が中央線に到達したときにそれをクローズします。
...
- **買いシグナル**: オープン ポジションがないときに、ローソク足の終値が Bollinger Band の上限に到達するか、それを上回る
- **売りシグナル** (ロング ポジションのクローズ): ロング ポジションがあるときに、ローソク足の終値が Bollinger Band の中央線に到達するか、それを下回る
...
- このストラテジーは Bollinger Bands インジケーターの上限バンドと中央線のみを使用します
- ロング ポジションのみが開かれます
