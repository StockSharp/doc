# 上限バンドに焦点を当てた Bollinger ストラテジー

## 概要

`BollingerStrategyUpBandStrategy` は、[BollingerBands](xref:StockSharp.Algo.Indicators.BollingerBands) インジケーターに基づくストラテジーです。価格がボリンジャーバンドの上限に到達したときにロング ポジションを開き、価格が中央線に到達したときにそれをクローズします。
...
- **ボリンジャー期間** - ボリンジャーバンド インジケーターの期間 (既定値 20)
- **ボリンジャー偏差** - 標準偏差の乗数 (既定値 2.0)
- **ローソク足タイプ** - 使用するローソク足の種類 (既定値 5 分足)
...
private void ProcessCandle(ICandleMessage candle, IIndicatorValue bollingerValue)
{
	...
	// 取引ロジック:
	// 価格が上限バンドに触れたときに買う (ポジションがない場合のみ)
	if (candle.ClosePrice >= typed.UpBand && Position == 0)
	{
		BuyMarket(Volume);
	}
	// 価格が中央線に到達したときにポジションをクローズするために売る (ロング ポジションがある場合のみ)
...
- **買いシグナル**: オープン ポジションがないときに、ローソク足の終値がボリンジャーバンドの上限に到達するか、それを上回る
- **売りシグナル** (ロング ポジションのクローズ): ロング ポジションがあるときに、ローソク足の終値がボリンジャーバンドの中央線に到達するか、それを下回る
...
- このストラテジーはボリンジャーバンド インジケーターの上限バンドと中央線のみを使用します
- ロング ポジションのみが開かれます
