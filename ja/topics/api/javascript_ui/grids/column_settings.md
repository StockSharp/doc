...
アダプターが扱うキーは**小文字**です。テーブルを解析するときの `data-col` の読み取りもこの形式で、外部へ渡すキー（`defaultKeys()` の戻り値と `write()` に渡す値）も同じ形式です。

> [!CAUTION]
> `apply()` に渡されるキーと `read()` から返されるキーは、この一覧と**そのまま、大文字小文字を区別して**照合されます。キー `Time` は検出済みの `time` と一致せず、不明なキーとして破棄されます。すべてのキーがそのように記録されていた場合は、表示される列が 1 つも残らず、コンソールにエラーも出ないままテーブルは管理対象列なしで開きます。ストアには `write()` で渡された値をそのまま保存し、キーを自分で組み立て直さないでください。

適用時には、不明なキーと重複したキーが破棄されます。空の配列は「管理対象列を 1 つも表示しない」という正当なレイアウトであり、「レイアウトが選択されていない」ことを意味するのは `null` だけです。
