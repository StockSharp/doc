...
このインジケーターを使用するには、[IntradayMomentumIndex](xref:StockSharp.Algo.Indicators.IntradayMomentumIndex) クラスを使用する必要があります。
...
2. Length 期間における正および負の変動の合計を計算します。
   ```
   上昇幅合計 = Length 期間のすべての上昇幅の合計
   下落幅合計 = Length 期間のすべての下落幅の合計
   ```
