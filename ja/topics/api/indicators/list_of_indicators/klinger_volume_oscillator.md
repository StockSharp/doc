...
KVO は、価格変動の方向と大きさ、および取引量の両方を考慮する出来高力インジケーターを使用して、価格情報と出来高を組み合わせます。その後、このマネーフローに 2 つの異なる期間の指数移動平均（EMA）を適用し、それらの差を計算します。
...
- **短期期間** - 短期 EMA を計算する期間（デフォルト値: 34）
- **長期期間** - 長期 EMA を計算する期間（デフォルト値: 55）

## 計算

クリンガー出来高オシレーターの計算には、いくつかの手順があります。
...
2. 出来高力インジケーターを計算します。
   ...
   ```
   (High - Low) がゼロの場合、出来高力は出来高にトレンドを掛けた値に設定されます。
