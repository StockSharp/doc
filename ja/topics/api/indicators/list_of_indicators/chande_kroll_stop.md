...
**チャンデ・クロール・ストップ (CKS)** は、Tushar Chande と Stanley Kroll によって開発されたストップロス水準を決定するためのインジケーターです。市場のボラティリティに適応し、トレーダーがポジションの手仕舞いポイントを設定するのに役立ちます。

このインジケーターを使用するには、[ChandeKrollStop](xref:StockSharp.Algo.Indicators.ChandeKrollStop) クラスを使用する必要があります。

## 説明

チャンデ・クロール・ストップ インジケーターは、市場のボラティリティとトレンドの変化に反応する、ストップロス水準を設定するための動的なツールとして開発されました。これは 2 本のラインで構成されます。上側のストップライン（ショートポジション用）と下側のストップライン（ロングポジション用）です。
...
- **期間** - 極値を計算するための主期間（既定値: 10）
- **乗数** - ATR の乗数で、極値からの距離を決定します（既定値: 1.5）
- **ストップ期間** - ストップ水準を計算するための期間（既定値: 20）

## 計算

チャンデ・クロール・ストップ の計算には、次の手順が含まれます。
...
4. StopPeriod に基づいて最終的なストップラインを決定します。
   ```
   上側ストップ = StopPeriod における上側バンドの最大値
   下側ストップ = StopPeriod における下側バンドの最小値
   ```

## 解釈

- **上側ストップ** はショートポジションに使用されます。終値が上側ストップを上回った場合、ショートポジションを決済する、またはロングポジションを建てるシグナルと見なすことができます。

- **下側ストップ** はロングポジションに使用されます。終値が下側ストップを下回った場合、ロングポジションを決済する、またはショートポジションを建てるシグナルと見なすことができます。
...
![CKS のチャート](../../../../images/indicator_chande_kroll_stop.png)
