...
**マクレラン・オシレーター (MCO)** は、値上がり銘柄と値下がり銘柄の移動平均の差を分析することで市場の広がりを測定する、Sherman McClellan と Marian McClellan によって開発されたテクニカル指標です。

この指標を使用するには、[McClellanOscillator](xref:StockSharp.Algo.Indicators.McClellanOscillator) クラスを使用する必要があります。

## 説明

マクレラン・オシレーター (MCO) は、市場全体の状態を評価し、潜在的な反転ポイントを特定するのに役立つ、最もよく知られた市場の広がり指標の 1 つです。1969 年に開発されて以来、多くのテクニカルアナリストにとって重要なツールとなっています。

MCO は、市場における上昇銘柄数と下落銘柄数の比率の分析に基づいています。この指標は、純上昇数（上昇銘柄数と下落銘柄数の差）の 19 期間および 39 期間の指数移動平均の差を計算します。

マクレラン・オシレーター は、特に次の用途に有用です。
- 市場全体の方向を判断する
...
マクレラン・オシレーター の計算には、次の手順が含まれます。

1. 各取引日の 純上昇数 を計算します。
   ```
   純上昇数 = 上昇銘柄 - 下落銘柄
   ```
   ここで、上昇銘柄 は上昇銘柄数、下落銘柄 は下落銘柄数です。

2. 純上昇数 の 19 期間指数移動平均を計算します。
   ```
   EMA19 = EMA(純上昇数, 19)
   ```

3. 純上昇数 の 39 期間指数移動平均を計算します。
   ```
   EMA39 = EMA(純上昇数, 39)
   ```

4. これら 2 つの EMA の差として マクレラン・オシレーター を計算します。
   ```
   MCO = EMA19 - EMA39
   ```
