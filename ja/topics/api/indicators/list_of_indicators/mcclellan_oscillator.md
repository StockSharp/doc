...
**マクレラン・オシレーター (MCO)** は、値上がり銘柄と値下がり銘柄の移動平均の差を分析することで市場の広がりを測定する、Sherman McClellan と Marian McClellan によって開発されたテクニカルインジケーターです。

このインジケーターを使用するには、[McClellanOscillator](xref:StockSharp.Algo.Indicators.McClellanOscillator) クラスを使用する必要があります。

## 説明

マクレラン・オシレーター (MCO) は、市場全体の状態を評価し、潜在的な反転ポイントを特定するのに役立つ、最もよく知られた市場の広がりインジケーターの 1 つです。1969 年に開発されて以来、多くのテクニカルアナリストにとって重要なツールとなっています。

MCO は、市場における上昇銘柄数と下落銘柄数の比率の分析に基づいています。このインジケーターは、純上昇数（上昇銘柄数と下落銘柄数の差）の 19 期間および 39 期間の指数移動平均の差を計算します。

マクレラン・オシレーターは、特に次の用途に有用です。
- 市場全体の方向を判断する
...
マクレラン・オシレーターの計算には、次の手順が含まれます。

1. 各取引日の純上昇数を計算します。
   ```
   純上昇数 = 上昇銘柄 - 下落銘柄
   ```
   ここで、上昇銘柄は上昇銘柄数、下落銘柄は下落銘柄数です。

2. 純上昇数の 19 期間指数移動平均を計算します。
   ```
   EMA19 = EMA(純上昇数, 19)
   ```

3. 純上昇数の 39 期間指数移動平均を計算します。
   ```
   EMA39 = EMA(純上昇数, 39)
   ```

4. これら 2 つの EMA の差としてマクレラン・オシレーターを計算します。
   ```
   MCO = EMA19 - EMA39
   ```
