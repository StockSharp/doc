...
**ハーモニックオシレーター (HO)** は、調和振動理論に基づくテクニカル指標で、価格変動に含まれる循環成分の特定に役立ちます。

この指標を使用するには、[HarmonicOscillator](xref:StockSharp.Algo.Indicators.HarmonicOscillator) クラスを使用する必要があります。

## 説明

ハーモニックオシレーター (HO) は、市場価格変動の周期性と循環的性質を特定するために開発された指標です。多くの価格変動には、分離して将来の価格変動予測に使用できる調和的 (周期的) 成分が含まれるという原理に基づいています。
...
4. 支配的なサイクルに基づく ハーモニックオシレーター の合成:
   ```
   HO = 逆 FFT による主周期の再構成
   ```
