...
**相対モメンタム指数 (RMI)** は、Roger Altman によって提案された従来の RSI インジケーターの修正版です。一定期間における価格の上昇と下落の比率を計算する古典的な RSI とは異なり、RMI は選択されたモメンタム期間における相対的な価格変化を考慮します。

このインジケーターを使用するには、[RelativeMomentumIndex](xref:StockSharp.Algo.Indicators.RelativeMomentumIndex) クラスを使用する必要があります。

## 説明

相対モメンタム指数 (RMI) は、モメンタム期間パラメーターを追加することで古典的な RSI を改善します。これにより、トレーダーは主計算期間を変更せずにインジケーターの感度を調整できます。
...
- **モメンタム期間** - 価格比較の時間ラグを定義するモメンタム期間。
- **期間** - インジケーターを計算するための主期間（RSI の期間に類似）。
