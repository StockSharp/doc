...
- **期間** - インデックスの主な計算期間（既定値: 14）
- **ストキャスティクスK期間** - ストキャスティクスオシレーター %K を計算するための期間（既定値: 5）
- **ストキャスティクスD期間** - ストキャスティクスオシレーター %D を計算するための期間（既定値: 3）
...
![CBCI のチャート](../../../../images/indicator_constance_brown_composite_index.png)
