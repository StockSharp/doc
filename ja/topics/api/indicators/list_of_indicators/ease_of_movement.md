...
**Ease of Movement (EMV)** は、Richard Arms によって開発されたテクニカル指標で、価格が上下に動く容易さを評価するために、価格変化と出来高を関連付けます。

この指標を使用するには、[EaseOfMovement](xref:StockSharp.Algo.Indicators.EaseOfMovement) クラスを使用する必要があります。

## 説明

Ease of Movement (EMV) 指標は、価格変動と出来高の関係を測定するために作成されました。この指標の主な概念は、上昇トレンドでは価格が少ない出来高で容易に上昇し、下降トレンドでは価格が同じく少ない出来高で容易に下落するべきだというものです。
...
Ease of Movement 指標の計算には、次の手順が含まれます。

1. Midpoint Move を計算します。
   ```
   Midpoint = (High + Low) / 2
   Midpoint Move = Midpoint[current] - Midpoint[previous]
   ```

2. Box Ratio（出来高と距離の係数）を計算します。
   ```
