...
**Vortex Indicator (VI)** は、2009年に Etienne Boisse と Julia Boisse によって開発されたテクニカルインジケーターです。このインジケーターは VI+ と VI- の2本のラインで構成され、上方向および下方向の価格変動を示し、新しいトレンドの始まりを特定し、既存のトレンドを確認するのに役立ちます。

このインジケーターを使用するには、[VortexIndicator](xref:StockSharp.Algo.Indicators.VortexIndicator) クラスを使用する必要があります。

## 説明

Vortex Indicator は自然界における渦の動きの原理に着想を得ており、市場の動きの周期的な性質を反映することを目的としています。これは2本のラインで構成されます。

- **VI+** (正の Vortex Indicator) - 上方向の価格変動を測定します
- **VI-** (負の Vortex Indicator) - 下方向の価格変動を測定します
...
Vortex Indicator は、特に次の用途に有用です。
- 新しいトレンドの開始を判断する
- 既存トレンドの強さを評価する
- 潜在的な反転ポイントを特定する
...
Vortex Indicator の計算は、いくつかの手順で実行されます。
