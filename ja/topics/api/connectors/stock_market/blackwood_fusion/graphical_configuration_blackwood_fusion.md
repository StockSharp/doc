# Blackwood (Fusion) のグラフィカル設定

すべての [S#](../../../../api.md) 製品では、接続のグラフィカル設定は [接続設定ウィンドウ](../../../graphical_user_interface/connection_settings_window.md) で行います。

![API GUI 設定 Fusion (Blackwood)](../../../../../images/api_gui_settings_fusion.png)

- **ログイン** - ログイン。
- **パスワード** - パスワード。
- **マーケットデータ** - マーケットデータサーバーのアドレス。
- **履歴** - 履歴データサーバーのアドレス。
- **トランザクション** - トランザクション実行サーバーのアドレス。
- **ログへのパス** - BWOrders.Log ファイルが作成されるディレクトリへのパス。
- **上書き** - リソース内の dll ファイルで上書きします。既定で有効です。
- **ハートビート** - 接続が有効かどうかを追跡するためのサーバーチェック間隔。既定では 1 分です。
- **再接続設定** - 取引システムとの接続を追跡するための設定メカニズム。([再接続設定](../../reconnection_settings.md))

## 推奨コンテンツ

[コネクター](../../../connectors.md)

[グラフィカル設定](../../graphical_configuration.md)

[独自コネクターの作成](../../creating_own_connector.md)

[設定の保存と読み込み](../../save_and_load_settings.md)

