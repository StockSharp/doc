> [!CAUTION]
> **このコネクターが使用する IEX 取引 API は利用できなくなりました。このコネクターは動作しません。ドキュメントは参照用としてのみ保持されています。**

# IEX

**Investors Exchange (IEX)** は、株式相場、Forex、ニュース、先物契約などのリアルタイムマーケットデータプロバイダーです。

現在の取引プラットフォーム向けに取引ロボットを書き始める前に、[コネクター](../../connectors.md) のリンクを読むことを推奨します。 

> [!CAUTION]
> コネクターはマーケットデータフィードのみをサポートし、トランザクションはサポートしていません。 

## 推奨コンテンツ

[コネクター](../../connectors.md)

[グラフィカル設定](../graphical_configuration.md)

[設定の保存と読み込み](../save_and_load_settings.md)

[独自コネクターの作成](../creating_own_connector.md)

[注文管理](../../orders_management.md)

[新規注文の作成](../../orders_management/create_new_order.md)

[新規逆指値注文の作成](../../orders_management/create_new_stop_order.md)

