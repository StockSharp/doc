...
- **Interval** - 接続試行が行われる間隔。 
- **Initially** - 初回接続が確立されなかった場合（タイムアウト、ネットワーク障害など）に、初回接続を確立する試行回数。 
- **Reconnection** - 動作中に接続が切断された場合に、再接続を試行する回数。 
- **Timeout** - 接続\/切断が成功するまでのタイムアウト。 
- **Operating mode** - 接続試行を行うべき動作モード。
