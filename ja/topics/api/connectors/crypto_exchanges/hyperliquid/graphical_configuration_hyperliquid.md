# Hyperliquid のグラフィカル設定

すべての [S#](../../../../api.md) 製品では、グラフィカル設定は [接続設定ウィンドウ](../../../graphical_user_interface/connection_settings_window.md) で行います。

主なフィールド:

 - **ウォレットアドレス** - ウォレットアドレス (`0x...`)。
 - **秘密鍵** - 署名付き操作用の秘密鍵。
 - **Vault アドレス** - 任意の vault またはサブアカウントアドレス。
 - **有効期限** - 任意の署名有効期限タイムスタンプ。
- **セクション** - 取引セクション (`Spot` または `Derivatives`)。
- **テストネット** - テストネットエンドポイント。
 - **Info エンドポイント / Exchange エンドポイント / WS エンドポイント** - エンドポイント URL。
- **成行スリッページ** - 成行注文の既定スリッページ。

## 推奨コンテンツ

[コネクタ](../../../connectors.md)

[グラフィカル設定](../../graphical_configuration.md)

[独自コネクタの作成](../../creating_own_connector.md)

[設定の保存と読み込み](../../save_and_load_settings.md)
