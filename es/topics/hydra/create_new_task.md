...
[Hydra](../hydra.md) está construida con la posibilidad de crear extensiones personalizadas. Para añadir su propia extensión, debe copiar la dll a la subcarpeta **Plugins** donde está instalada [Hydra](../hydra.md). A continuación se muestra el proceso de creación de una utilidad para hacer copias de seguridad de datos existentes (se incluye de serie y se presenta como material de aprendizaje).

Cada utilidad debe implementar la interfaz [IHydraTask](xref:StockSharp.Hydra.Core.IHydraTask) (o heredar de las clases [BaseHydraTask](xref:StockSharp.Hydra.Core.BaseHydraTask) o [ConnectorHydraTask\`1](xref:StockSharp.Hydra.Core.ConnectorHydraTask`1)): 
...
Un paso importante al crear la utilidad es implementar el método [BaseHydraTask.OnProcess](xref:StockSharp.Hydra.Core.BaseHydraTask.OnProcess), que contiene la lógica de la utilidad. En el caso de implementar copia de seguridad de datos, toda la lógica está dentro de la [API](../api.md): 
