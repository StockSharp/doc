...
La principal fortaleza de OMA radica en su capacidad para identificar cambios en el impulso de la tendencia y generar señales de negociación basadas en cruces de línea cero y divergencias de precios.
...
1. Calcule la media móvil corta:
   ```
   media móvil corta = SMA(Price, ShortPeriod)
   ```

2. Calcule la media móvil larga:
   ```
   media móvil larga = SMA(Price, LongPeriod)
   ```

3. Calcule OMA como la diferencia entre medias móviles cortas y largas:
   ```
   OMA = media móvil corta - media móvil larga
   ```
...
![Gráfico del indicador OMA](../../../../images/indicator_oscillator_of_moving_average.png)
