...
MI mide la volatilidad utilizando medias móviles exponenciales (EMA) del rango de precios. No predice la dirección de la inversión, solo su probabilidad. Es por eso que MI se usa a menudo junto con otros indicadores direccionales.
...
El cálculo del Índice de masa implica los siguientes pasos:
...
2. Calcule la EMA de 9 períodos del rango:
   ```
   EMA1 = EMA(Range, EmaLength)
   ```

3. Calcule la EMA de 9 períodos de la EMA del rango:
   ```
   EMA2 = EMA(EMA1, EmaLength)
   ```

4. Calcule la relación:
   ```
   Ratio = EMA1 / EMA2
   ```

5. Sume las relaciones de 25 períodos:
   ```
   MI = Sum(Relación durante los últimos períodos Length)
   ```
...
- EmaLength - período de la EMA (normalmente 9)
...
2. **Niveles de índice**:
   ...
   - Los valores altos seguidos de una caída pueden preceder a un cambio de tendencia
   - Los valores bajos (por debajo de 20) indican una contracción del rango de precios y una volatilidad reducida
