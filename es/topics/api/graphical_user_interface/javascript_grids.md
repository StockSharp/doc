...
- [DataGrid](javascript_grids/data_grid.md) construye el encabezado y las filas a partir de una única descripción de columnas. Gestiona la ordenación, los filtros, la agrupación, la selección de filas, los totales fijados, el menú contextual, la persistencia del estado y la exportación.
- [ColumnSettings](javascript_grids/column_settings.md) se conecta a una tabla HTML ya renderizada por el servidor y permite al usuario cambiar el orden y la visibilidad de las columnas.
- [TableSort](javascript_grids/table_sort.md) proporciona un controlador de ordenación independiente para una tabla administrada por la aplicación.
- [TableExport](javascript_grids/table_export.md) genera un libro OOXML `.xlsx` sin bibliotecas de terceros.
