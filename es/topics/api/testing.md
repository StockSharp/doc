...
3. Pruebas con [simulador](testing/simulator.md) basadas en datos recibidos desde una conexión real al sistema de negociación (por ejemplo, desde [OpenECry](connectors/stock_market/openecry.md)), pero sin registro real de órdenes (la ejecución se emula en función de los libros de órdenes entrantes).

Al usar los tres modos, se pone el máximo énfasis en que el código de la estrategia escrito con [Strategy](xref:StockSharp.Algo.Strategies.Strategy) no cambie al pasar de negociación real a pruebas y viceversa. Esto se logra mediante la implementación de la interfaz principal [IConnector](xref:StockSharp.BusinessEntities.IConnector), que actúa como gateway al sistema de negociación. Cómo se usa la interfaz ya se mostró en la sección [API](../api.md). En modo de pruebas, no actúa un sistema de negociación real, sino la emulación (según el modo seleccionado). Por lo tanto, el código de la estrategia nunca sabrá si está operando con una bolsa real o con una emulación.
