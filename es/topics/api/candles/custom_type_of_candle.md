   ...
   public static class DeltaCandleHelper
   {
       ...
       public static void RegisterDeltaCandleType()
       {
           ...
           Extensions.RegisterCandleType<decimal>(
               typeof(DeltaCandleMessage),      // Tipo de mensaje de vela
               DeltaCandleType,                // Tipo de mensaje
               "delta",                        // Nombre de archivo para almacenamiento
               str => str.To<decimal>(),       // Convertidor de cadena a parámetro
               arg => arg.ToString(),          // Convertidor de parámetro a cadena
               a => a > 0,                     // Validador de parámetros
               false                           // Si estas velas pueden obtenerse de la fuente (no solo construirse)
           );
       }
   }
   ```
...
public class DeltaCandleStrategy : Strategy
{
	...
	public DeltaCandleStrategy()
	{
		...
		_deltaThreshold = Param(nameof(DeltaThreshold), 1000m)
			.SetDisplay("Valor umbral de delta", "Valor delta de volumen para la formación de velas", "Configuración principal")
			.SetGreaterThanZero()
			.SetCanOptimize(true)
		...
		_signalDelta = Param(nameof(SignalDelta), 500m)
			.SetDisplay("Delta mínimo para señal", "Valor delta mínimo para la generación de señales", "Configuración principal")
			.SetGreaterThanZero()
			.SetCanOptimize(true);
