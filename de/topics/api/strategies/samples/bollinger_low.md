# Bollinger-Strategie mit Fokus auf das untere Band

## Überblick

`BollingerStrategyLowBandStrategy` ist eine Strategie auf Basis des Indikators [BollingerBands](xref:StockSharp.Algo.Indicators.BollingerBands). Sie eröffnet eine Short-Position, wenn der Preis die untere Begrenzung der Bollinger-Bänder erreicht, und schließt sie, wenn der Preis die Mittellinie erreicht.
...
private void ProcessCandle(ICandleMessage candle, IIndicatorValue bollingerValue)
{
	...
	// Handelslogik:
	// Verkaufen, wenn der Preis das untere Band berührt (nur wenn keine Position besteht)
	if (candle.ClosePrice <= typed.LowBand && Position == 0)
	{
		SellMarket(Volume);
	}
	// Kaufen, um die Position zu schließen, wenn der Preis die Mittellinie erreicht (nur bei Short-Position)
...
- **Verkaufssignal**: Der Schlusskurs der Kerze erreicht das untere Bollinger-Band oder fällt darunter, wenn keine offene Position vorhanden ist.
- **Kaufsignal** (Schließen der Short-Position): Der Schlusskurs der Kerze erreicht oder überschreitet die Mittellinie der Bollinger-Bänder, wenn eine Short-Position vorhanden ist.
...
- Die Strategie verwendet nur das untere Band und die Mittellinie des Bollinger-Bänder-Indikators.
- Es werden nur Short-Positionen eröffnet.
