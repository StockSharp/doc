...
- **Entfernen nicht benötigter Regeln.**
  ...
  ruleCanceled
      .Do(() =>
      {
          this.AddInfoLog("Order erfolgreich storniert");
  ...
  order
      ...
      .Do(() => this.AddInfoLog("Order wurde von der Börse nicht akzeptiert"))
      .Once()
      .Apply(this);
  order
      .WhenMatched(Connector)
      .Do(() => this.AddInfoLog("Order vollständig ausgeführt"))
      .Once()
      .Apply(this);
  // Orderregistrierung
  RegisterOrder(order);
...
- **Kombinieren von Regeln mit der Bedingung [MarketRuleHelper.Or](xref:StockSharp.Algo.MarketRuleHelper.Or(StockSharp.Algo.IMarketRule,StockSharp.Algo.IMarketRule[]))**(**[StockSharp.Algo.IMarketRule](xref:StockSharp.Algo.IMarketRule) rule, [StockSharp.Algo.IMarketRule\[\]](xref:StockSharp.Algo.IMarketRule[]) rules **)** / [MarketRuleHelper.And](xref:StockSharp.Algo.MarketRuleHelper.And(StockSharp.Algo.IMarketRule,StockSharp.Algo.IMarketRule[]))**(**[StockSharp.Algo.IMarketRule](xref:StockSharp.Algo.IMarketRule) rule, [StockSharp.Algo.IMarketRule\[\]](xref:StockSharp.Algo.IMarketRule[]) rules **)**.**
  ...
  Connector
      ...
      .Do(() => this.AddInfoLog("Kerze geschlossen oder Zeit abgelaufen"))
      .Once()
      .Apply(this);
...
{
	LogInfo($"Regel WhenOrderBookReceived #1 BestBid={depth.GetBestBid()}, BestAsk={depth.GetBestAsk()}");
}).Once().Apply(this);
...
{
	LogInfo($"Regel WhenOrderBookReceived #2 BestBid={depth.GetBestBid()}, BestAsk={depth.GetBestAsk()}");
}).Once().Apply(this);

// Regel innerhalb einer Regel
mdSub.WhenOrderBookReceived(this).Do((depth) =>
{
	LogInfo($"Regel WhenOrderBookReceived #3 BestBid={depth.GetBestBid()}, BestAsk={depth.GetBestAsk()}");

	// Regel ohne Angabe von Once()
	mdSub.WhenOrderBookReceived(this).Do((depth1) =>
	{
		LogInfo($"Regel WhenOrderBookReceived #4 BestBid={depth1.GetBestBid()}, BestAsk={depth1.GetBestAsk()}");
	}).Apply(this);
}).Once().Apply(this);
...
sub.WhenTickTradeReceived(this).Do(t =>
{
	...
	sub
		...
		.Do(t =>
		{
			LogInfo($"Regel WhenLastTradePriceMore oder WhenLastTradePriceLess ausgelöst: Tick={t}");
		})
		.Apply(this);
})
