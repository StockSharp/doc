# Adapter-Initialisierung: SinoPac Shioaji

Der folgende Code zeigt, wie [ShioajiMessageAdapter](xref:StockSharp.Shioaji.ShioajiMessageAdapter) initialisiert und zu [Connector](xref:StockSharp.Algo.Connector) hinzugefügt wird.

```cs
var messageAdapter = new ShioajiMessageAdapter(Connector.TransactionIdGenerator)
{
	Key = "<Wert>".ToSecureString(),
	Secret = "<Wert>".ToSecureString(),
	Address = "<Wert>",
	ReconnectAttempts = 10,
};

Connector.Adapter.InnerAdapters.Add(messageAdapter);
```

Ersetzen Sie die Beispielwerte durch die für Ihr Konto ausgegebenen oder konfigurierten Parameter.

## Siehe auch

[Fenster für Verbindungseinstellungen](../../../graphical_user_interface/connection_settings_window.md)
