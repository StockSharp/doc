...
- [DataGrid](grids/data_grid.md) erstellt Überschrift und Zeilen anhand einer gemeinsamen Spaltendefinition. Die Komponente übernimmt Sortierung, Filterung, Gruppierung, Zeilenauswahl, angeheftete Ergebniszeilen, Kontextmenü, Zustandsspeicherung und Export.
- [ColumnSettings](grids/column_settings.md) bindet sich an eine bereits serverseitig gerenderte HTML-Tabelle und ermöglicht es Benutzern, Reihenfolge und Sichtbarkeit der Spalten zu ändern.
- [TableSort](grids/table_sort.md) stellt einen eigenständigen Sortiercontroller für eine vom Anwendungsprogramm verwaltete Tabelle bereit.
- [TableExport](grids/table_export.md) erzeugt eine OOXML-Arbeitsmappe im Format `.xlsx` ohne externe Bibliothek.
...
Der Bereich `@1` nimmt die neueste Version des ersten Hauptzweigs. Eine genaue Nummer festzunageln lohnt sich nur dann, wenn der Build absichtlich reproduzierbar sein soll: Eine festgepinnte alte Version liefert stillschweigend einen Build ohne die seither hinzugekommenen Möglichkeiten, und das Fehlen von etwa `groupOrder` zeigt sich nicht dort, wo dieses Tag steht.

Die rohen TypeScript-Quellen stehen über die Unterpfade `@stocksharp/grids/source` und `@stocksharp/grids/source/<Modul>` bereit — sie werden benötigt, wenn die Tabellen von Ihrem Bundler in einem Durchgang mit dem übrigen Code gebaut werden.
