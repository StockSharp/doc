...
2. Importparameter für [S#](../../api.md)-Felder konfigurieren.
   - **S#-Feld** - Wert des S#-Feldes (**Instrument**, **Handelsplatz** usw.).
   - **Zuordnungen** - Spaltenwert in der Datei dem StockSharp-Typ zuordnen (falls erforderlich).
   - **Format** - Datenformat. Wird typischerweise beim Import von Datums- und Zeitwerten verwendet (siehe [Trades](ticks.md)).
   - **Verwenden** - ob die Daten beim Import verwendet werden sollen.
   - **Feldreihenfolge** - Reihenfolge, in der die Eigenschaftsspalten des importierten Elements angeordnet sind.
     ...
     Der Wert **Instrument** entspricht **{SecurityId.SecurityCode}** mit der fortlaufenden Nummer **0**.

     > [!TIP]
     > In der Programmierung ist die Ordnungsnummer des ersten Elements immer 0

     Der Wert **Handelsplatz** entspricht **{SecurityId.BoardCode}** mit der fortlaufenden Nummer **1**. Und so weiter.
   - **Standardmäßig** - Standardwert des Feldes. Er kann zum Beispiel für wiederholte Feldwerte verwendet werden (**Instrument** oder **Handelsplatz** beim Import von Trades, Orderbüchern usw., siehe [Trades](ticks.md)), wenn die entsprechende Information in der Datendatei fehlt.
...
3. Um eine Vorschau der Daten anzuzeigen, klicken Sie auf die Schaltfläche **Vorschau**.![hydra import preview candles](../../../images/hydra_import_preview_candles.png)
