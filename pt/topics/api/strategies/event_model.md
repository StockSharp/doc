...
Na [API](../../api.md), existem várias condições e ações predefinidas para [IMarketRule](xref:StockSharp.Algo.IMarketRule), adaptadas a cenários comuns. Estas são adicionadas como métodos de extensão em [MarketRuleHelper](xref:StockSharp.Algo.MarketRuleHelper).
