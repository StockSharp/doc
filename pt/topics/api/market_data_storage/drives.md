...
- **Sistema de ficheiros** — os dados são organizados numa estrutura hierárquica de diretórios por instrumentos e datas.
...
- **Segurança de threads** — o acesso aos dados é protegido por mecanismos de bloqueio para funcionamento correto em aplicações multithread.
...
- `Credentials` — credenciais de autenticação (e-mail e palavra-passe).
...
A classe [DriveCache](xref:StockSharp.Algo.Storages.DriveCache) gere uma coleção de unidades de armazenamento e fornece cache para reutilização.
...
- **Changed** — evento para alterações na coleção de unidades.
