...
if (posChange != null)
{
    Console.WriteLine($"Posição: {posChange.CurrentValue}");
}
```
...
{
    Console.WriteLine($"Operação completa concluída:");
    Console.WriteLine($"  Aberta: {report.OpenTime}");
    Console.WriteLine($"  Fechada: {report.CloseTime}");
};
