...
Alguns indicadores são compostos e usam outros indicadores nos seus cálculos. Por isso, os indicadores podem ser reutilizados entre si, como demonstra o exemplo de implementação do indicador Volatilidade de Chaikin [ChaikinVolatility](xref:StockSharp.Algo.Indicators.ChaikinVolatility):
...
{
	/// <summary>
	/// Cria <see cref="ChaikinVolatility"/>.
	/// </summary>
...
Abaixo está um exemplo de implementação do oscilador percentual de volume (PVO), que demonstra a implementação de `NumValuesToInitialize`, `Measure` e dos métodos `Save` e `Load`:

```cs
/// <summary>
/// Oscilador percentual de volume (PVO).
/// </summary>
[Display(
...
public class PercentageVolumeOscillatorValue : ComplexIndicatorValue<PercentageVolumeOscillator>
{
	...
	/// <param name="time">Hora do valor.</param>
