# Sterling

**Sterling** — plataforma de negociação para negociar em bolsas internacionais.

Antes de começar a escrever robôs de negociação para a plataforma de negociação atual, recomenda-se ler as ligações na secção [Conectores](../../connectors.md). 

## Conteúdo recomendado

[Conectores](../../connectors.md)

[Configuração gráfica](../graphical_configuration.md)

[Guardar e carregar definições](../save_and_load_settings.md)

[Criar o próprio conector](../creating_own_connector.md)

[Gestão de ordens](../../orders_management.md)

[Criar nova ordem](../../orders_management/create_new_order.md)

[Criar nova ordem stop](../../orders_management/create_new_stop_order.md)

[Inicialização do adaptador Sterling](sterling/adapter_initialization_sterling.md)
