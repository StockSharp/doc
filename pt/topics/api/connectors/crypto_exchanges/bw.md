> [!CAUTION]
> **A bolsa BW encerrou as operações. O conector não funciona mais; a documentação é mantida apenas para referência.**

# BW

**BW** - bolsa de criptomoedas.

Antes de começar a escrever robôs de negociação para esta plataforma de negociação, recomendamos a leitura dos links na seção [Conectores](../../connectors.md).

## Conteúdo recomendado

[Conectores](../../connectors.md)

[Configuração gráfica](../graphical_configuration.md)

[Salvar e carregar configurações](../save_and_load_settings.md)

[Criando Seu Próprio Conector](../creating_own_connector.md)

[Gestão de Ordens](../../orders_management.md)

[Criar nova ordem](../../orders_management/create_new_order.md)

[Criar nova ordem stop](../../orders_management/create_new_stop_order.md)
