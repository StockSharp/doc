...
- [DataGrid](grids/data_grid.md) constrói o cabeçalho e as linhas a partir de uma descrição unificada das colunas. É responsável pela ordenação, filtros, agrupamento, seleção de linhas, totais fixos, menu de contexto, persistência do estado e exportação.
- [ColumnSettings](grids/column_settings.md) liga-se a uma tabela HTML já apresentada pelo servidor e permite ao utilizador alterar a ordem e a visibilidade das colunas.
- [TableSort](grids/table_sort.md) fornece um controlador de ordenação autónomo para uma tabela gerida pela aplicação.
- [TableExport](grids/table_export.md) cria um livro OOXML `.xlsx` sem bibliotecas externas.
...
O intervalo `@1` toma a versão mais recente do primeiro ramo maior. Só vale a pena fixar um número exato quando a compilação é reproduzida de propósito: uma versão antiga fixada entrega em silêncio uma compilação sem as funcionalidades entretanto surgidas, e a falta de, por exemplo, `groupOrder` manifesta-se longe do sítio onde esta etiqueta está escrita.

O código-fonte em TypeScript está acessível através dos subcaminhos `@stocksharp/grids/source` e `@stocksharp/grids/source/<módulo>` — são necessários quando as tabelas são compiladas pelo seu empacotador na mesma passagem que o restante código.
...
- [Gráficos JavaScript](charts.md)
- [Diagrama JavaScript](diagram.md)
