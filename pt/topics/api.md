...
A API do StockSharp, também conhecida como API S#, é um kit de desenvolvimento de software (SDK) para construir aplicações de negociação como [Designer](designer.md), [Terminal](terminal.md) e outras ferramentas de negociação personalizadas. Ela fornece a infraestrutura central para dados de mercado, roteamento de ordens, execução de estratégias, testes e componentes de interface de usuário para negociação.

## Recursos

- **Scripting de estratégias**: A API do StockSharp permite que os usuários escrevam e executem estratégias de negociação diretamente no [Designer](designer/strategies/using_code.md). As estratégias podem ser desenvolvidas, testadas e implantadas usando C#, F# ou Python.
