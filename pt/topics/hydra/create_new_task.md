...
[Hydra](../hydra.md) foi concebido com a possibilidade de criar extensões personalizadas. Para adicionar a sua própria extensão, é necessário copiar a dll para a subpasta **Plugins** onde [Hydra](../hydra.md) está instalado. Abaixo é apresentado o processo de criação de um utilitário para fazer cópias de segurança dos dados existentes (incluído por predefinição e apresentado como material de formação).

Cada utilitário tem de implementar a interface [IHydraTask](xref:StockSharp.Hydra.Core.IHydraTask) (ou herdar das classes [BaseHydraTask](xref:StockSharp.Hydra.Core.BaseHydraTask) ou [ConnectorHydraTask\`1](xref:StockSharp.Hydra.Core.ConnectorHydraTask`1)):
...
Um passo importante na criação do utilitário é a implementação do método [BaseHydraTask.OnProcess](xref:StockSharp.Hydra.Core.BaseHydraTask.OnProcess), que contém a lógica do utilitário. No caso da implementação da cópia de segurança dos dados, toda a lógica está dentro da [API](../api.md):
