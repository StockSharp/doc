# Configuração de notificações

Para definir uma notificação, tem de clicar no botão ![Designer botão Adicionar 00](../../../images/designer_creation_tool_00.png) no painel de notificações ([Painel de notificações](notification_panel.md)) ou clicar no botão ![Designer sino de alerta](../../../images/designer_alert_bell.png) diretamente nos painéis que suportam notificações ([Carteiras](../user_interface/portfolios.md), [Fluxo de negócios](../user_interface/components/trades_feed.md), [Notícias](../user_interface/components/news.md), [Level 1](../user_interface/components/level_1.md)).

![Designer Configuração de notificações](../../../images/designer_notifications_setting.png)

As notificações podem ser configuradas para alterações nos seguintes tipos de dados: carteira, código do cliente, corretor, depositário, hora do servidor, negócio, tipo de dados, cancelamento, ID da ordem, ID da ordem (string), ID da ordem (quadro), derivado, derivado (string), preço, volume (ordem), volume (negócio), volume visível, direção, remanescente, tipo de ordem, estado, comentário, mensagem da ordem, ordem do sistema, hora de expiração da ordem, condição de execução, ID da ordem, ID da ordem (string), preço, iniciador do negócio, interesse aberto, erro, condição, tendência de alta, comissão, atraso, derrapagem, ID (utilizador), moeda, P\/L, posição, formador de mercado.

As notificações podem ter as seguintes formas:

- **Janela** – será apresentada uma pequena janela pop-up com uma mensagem no canto do ecrã.
- **Melodia** – será reproduzida uma melodia.
- **SMS** – será enviada uma mensagem por SMS.
- **Email** – a mensagem será enviada por email.
- **Voz** – a mensagem será lida por uma voz gerada pelo computador.
- **Registo** – a mensagem será enviada para a janela de registos do Designer.
- **Desligado** – a notificação não será apresentada.
