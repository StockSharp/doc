...
O tipo de dados **Instrument** é seleccionado para o cubo [Variável](../elements/data_sources/variable.md). Se o instrumento não for especificado, mas a flag **Parameters** do grupo **Common** estiver definida, então será retirado da estratégia e depois passado para [Posição](../elements/positions/current.md).

Para o cubo [Posição](../elements/positions/current.md), a propriedade da posição também não é especificada, mas a flag **Parameters** do grupo **Common** está definida, o que significa que a posição será obtida para a carteira especificada nas definições da estratégia.
