...
- **Saldo**: Este socket transmite informação sobre a parte da posição que não foi realizada no fim da operação de modificação da posição. O valor devolvido pelo socket indica o resultado da operação:
...
- **Direção**: Especifica a direcção para "OpenPosition" e "None", e serve como filtro opcional para outras condições.
- **Algoritmo**: As opções incluem "Market Order", "VWAP", "Iceberg".
- **Parte**: A fracção do volume total que será dividida em segmentos mais pequenos ao utilizar algoritmos como "VWAP" ou "Iceberg".

Se o componente receber um trigger quando já iniciou a alteração do volume, ignora o novo trigger. Se as condições de modificação forem incompatíveis com o estado actual da posição (por exemplo, tentar "OpenPosition" quando já existe uma posição aberta), o componente devolve imediatamente `-1` através do socket de saída **Saldo**, indicando que a operação não é necessária e não foi iniciada.
