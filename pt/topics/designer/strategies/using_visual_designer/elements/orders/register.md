...
O componente "Registo de ordem" é utilizado para colocar ordens de negociação para um instrumento selecionado.

## Conectores de entrada

- **Instrumento** - O instrumento selecionado para a ordem.
- **Preço** - Especifica o preço de uma ordem limitada.
- **Acionador** - Sinal de ativação da ordem; aceita qualquer valor excepto `False`.
...
- **Transação** - Informação sobre a transação efectuada na ordem.
...
- **Validade** - A duração durante a qual uma ordem limitada permanece ativa.

## Configuração de ordem condicional

**Ordem condicional** - Uma ordem com condições adicionais que determinam o momento de colocação no sistema de negociação com base na situação atual do mercado.

![Captura de ecrã de Registo de ordem](../../../../../../images/designer_conditional_application.png)
...
- **Preço stop** - O preço stop que define a condição de ativação da ordem stop.
...
- **Direção da ordem condicional por execução** - A direção da ordem condicional baseada na execução.
- **Ativação em execução parcial** - A execução parcial da ordem é considerada. Uma ordem "por execução" será ativada após a execução parcial da ordem de condição.
...
- **Desvio do máximo** - O valor de desvio face ao preço máximo (mínimo) da última transação.
- **Spread de proteção** - O tamanho do spread de proteção.
